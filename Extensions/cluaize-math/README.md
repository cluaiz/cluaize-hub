# 🧩 Cluaize Math Extension (WASM Sandbox Demo)

This extension is the **WASM Sandboxed** dynamic plugin template for Cluaize.

## Overview
Unlike `cluaize-db` (which is a native compiled `.dll` with unrestricted OS access), `cluaize-math` represents a **Non-Built (Hot-Loaded) Extension** designed for safety and ease of distribution.
...
1. The user copies the plugin folder containing `manifest.yaml` and `bin/plugin.wasm` directly into `.cluaize/extensions/`.
2. Upon engine boot, `CluaizeExtensionRegistry` parses `manifest.yaml` and checks that the `sandbox_type` is `WASM`.
